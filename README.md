...
- **🗄 数据库实例管理**
  ...
  - 详情页已按引擎区分展示：SQL 实例保留表结构属性，Redis 实例仅保留结构浏览与查询控制台。
...
- **数据库实例管理与详情页修复**
  ...
  - 详情页已精简为“结构浏览 + 查询控制台”，其中 Redis 不再展示“表结构属性”，所有引擎均移除了“数据预览”模块。
...
| `database-instance-detail.png` | 数据库实例详情 | 结构浏览、SQL/Redis 查询控制台、结果表格 |
