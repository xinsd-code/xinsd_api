...
- **📊 NL2DATA 自然语言取数工作台**
  - 新增独立 `NL2DATA` 页面，支持通过 AI Chat 从自然语言问句生成 SQL，并在同一工作台内直接执行取数。
  - 已升级为“两阶段链路”：第一轮先做 NER 名词识别，第二轮再基于识别结果生成 SQL，提升业务名词映射准确率。
  - 第一轮 NER 支持结合指标配置中的“名词识别”字段补充业务语料，并对候选名词做去重、压缩与 Top N 裁剪，避免上下文过长。
  - 工作区采用 `取数 SQL + 会话历史 + 数据内容` 的并列布局，支持展示当前用户问句、SQL 与查询结果表格。
  - 会话历史已持久化到 SQLite，记录 `用户问句 / SQL / Prompt`，页面重新进入后可直接恢复最近会话。
  - 点击历史记录可打开详情面板，查看完整问句、SQL、Prompt，并通过“同步 SQL”一键回填到当前取数区继续编辑或重跑。
  - 重新执行当前 SQL 时只刷新取数结果，不会重复写入相同会话历史。
...
- **🧩 模型管理（OpenAI Compatible）**
  ...
  - 模型管理现已区分 `chat` 与 `embedding` 两类模型，可分别新增、编辑并设置各自默认模型。
  - `embedding` 类型已支持真实校验 `/embeddings` 请求链路，便于后续文本向量化能力接入。
...
- **🗄 数据库实例管理**
  ...
  - “指标配置”页支持导入字段备注，并可为字段开启“名词识别”参与开关，用于补充 NL2DATA 的第一轮业务名词识别语料。
...
- **DB Harness 多智能体链路与可视化增强**
  - 新增 `DB Harness` 多智能体链路工作台（Intent / Schema / Query / Guardrail / Analysis），支持在对话中展示完整执行链路与阶段状态。
  - 链路详情已补充从“自然语言理解”到“数据执行”之间的核心中间产物，并支持在链路中查看 `Catalog / Semantic / Query Plan`。
  - Agent 节点悬浮信息已优化为不影响主链路排版的气泡展示，长内容支持完整查看，避免被侧栏裁切。

- **语义配置统一为单一入口**
  - 原“指标配置 + 语义配置”双配置模式已收敛为“仅语义配置”，避免重复维护同一语义信息。
  - 语义配置页改为图形化逐字段编辑，支持语义名称、描述、别名、语义角色、计算口径与 NER 开关。
  - 语义配置保存后会自动派生系统可消费的有效映射，`DB Harness / NL2DATA / DB API 编辑器 / 数据库实例详情` 等消费链路已统一切换到语义优先。
  - 语义配置 UI 已完成多轮细节优化：两行字段布局、说明气泡化、名词识别控件与普通字段样式对齐。

- **数据库实例与语义链路稳定性修复**
  - 修复 MySQL 实例详情读取 schema 时 `Column 'table_name' in field list is ambiguous` 报错问题。
  - 增强 schema 元数据能力，补充字段关联信息（外键关系）用于目录与语义上下文构建。
  - 新增语义模型接口与实例级持久化能力，支持手动刷新语义模型与在线编辑回写。

- **NL2DATA 名词识别增强**
  - 新增 `docs/nl2data-ner-enhancement.md`，记录 NL2DATA 从单阶段 SQL 生成升级为“两阶段 NER + SQL”的设计方案。
  - 指标配置页新增“名词识别”开关，可将选中的字段指标补充到第一轮 NER 候选集。
  - NL2DATA 后端已拆分为第一轮 `NER Prompt` 与第二轮 `SQL Prompt`，Prompt Debug 区会完整展示两阶段上下文。
  - 第一轮候选语料已接入本地召回、去重、压缩和限额裁剪策略，避免勾选名词过多时 prompt 失控。

- **模型管理支持 Embedding**
  - 模型来源现支持 `chat / embedding` 两种类型，并允许分别设置默认模型。
  - 现有三个 AI Chat 入口仍只展示 `chat` 模型，embedding 模型不会进入对话模型选择器。
  - embedding 配置已接入真实接口校验，便于后续文本向量化场景直接复用。

- **指标配置与低分辨率 UI 修复**
  - 指标配置页支持把数据库字段备注一键导入为“指标名称 / 指标描述”，并修复了“已配置字段数”统计错误。
  - “名词识别”控件已收敛到与其他字段一致的表单节奏，并优化为轻量开关 + 左侧悬浮提示气泡。
  - 修复了低宽度窗口下字段卡片右侧输入项越界的问题，已补充分段响应式布局并完成窄屏自测。

- **生产部署与运维能力补齐**
  - 新增 `docs/deploy-production.md`，补充 Linux 服务器上的完整部署流程，覆盖 `git pull`、`npm ci`、`npm run build`、`PM2` 启动与 `Nginx` 反向代理。
  - 仓库新增 `ecosystem.config.js`，可直接用 `PM2` 以 `npm start` 方式托管生产进程。
  - 新增 `GET /api/health` 健康检查接口，返回当前环境、SQLite 路径、数据库文件是否存在等信息，便于做存活检测与部署验收。

- **SQLite 生产目录配置升级**
  - `.env.example` 已改为面向生产部署的示例，补充 `NODE_ENV`、`PORT`、`HOSTNAME`、`DATA_DIR`、`SQLITE_DB_PATH` 等配置项。
  - SQLite 默认不再强依赖项目根目录，服务启动时会自动创建数据库所在目录，更适合服务器部署与数据目录持久化。
  - `package.json` 已声明 Node.js 版本要求为 `>=20 <21`，并同步将 TypeScript 编译目标提升到 `ES2018`。

- **页面稳定性与默认值修复**
  - `数据库实例` 与 `指标配置` 页面已为 `useSearchParams()` 补齐 `Suspense` 包裹，规避构建与运行时警告。
  - `DB API` 详情回填时补齐默认分组与默认 Redis 配置，避免历史数据缺字段时页面状态异常。
  - Mock 首页在接口列表请求失败或返回结构异常时会安全降级为空数组，不再把页面留在不稳定状态。

- **SQL AI Chat 与 SQL 编辑体验升级**
  - SQL AI Chat 面板改为更适合长对话的双栏布局，消息区、Prompt Debug 区与最近一次生成 SQL 均支持更合理的滚动与收起展开。
  - 用于生成 SQL 的模型上下文已做压缩：会优先保留当前 SQL、关键入参、核心库表与指标摘要，减少无效 JSON / schema 噪声。
  - SQL 编辑页顶部数据库信息改为直接显示数据库名称；指标 hover 提示仅展示字段与指标信息，不再重复展示表说明。
  - DB API 主配置页中的 SQL 配置概览支持滚动查看长 SQL；MySQL 预览执行前会自动修正被异常拆分的中文别名，避免预览阶段 SQL 直接报错。

- **NL2DATA 工作台上线**
  - 新增 `/nl2data` 自然语言取数控制台，默认提供 `AI Chat` 入口、用户问句展示区、SQL 编辑区、会话历史区与结果预览区。
  - 会话历史改为数据库持久化，支持保存 `用户问句 / SQL / Prompt`，切回页面后会自动加载已保存记录。
  - 历史列表只展示问句与 SQL 摘要，点击后以详情交互框呈现完整内容，并支持“同步 SQL”回填到当前取数区。
  - 数据内容区表格样式已单独优化，结果条数标签移动到标题右侧；重新执行当前 SQL 时不会再额外生成重复历史。

- **未保存修改拦截规则统一**
  - API Client、API Forward、DB API、SQL 编辑页、模型管理、数据库实例与指标配置页，均已接入统一的“未保存修改”离开提醒。
  - 页面发生跳转时支持三种选择：继续编辑、不保存直接离开、保存后离开；保存失败时会停留当前页。
  - 拦截范围已覆盖页面内按钮跳转、站内链接点击与浏览器原生返回。
  - 父页面进入子页面时已默认放行，例如 `DB API 主配置页 -> SQL 编辑页`，不再额外弹确认框。
  - 当页面实际没有变动时不会触发确认弹框；从自定义弹框确认离开后，也不会再出现浏览器原生 `beforeunload` 二次弹窗。

- **Mock 接口工作台交互修复**
  - Mock 列表页点击某一条“编辑”时，加载动画现在只作用于当前被点击的那一项，不会再让所有“编辑”按钮一起进入加载态。
...
### 6. NL2DATA 工作台
- 适合通过自然语言快速生成分析 SQL，并直接在页面内完成调试、执行与结果查看。
- 左侧展示当前用户问句与 SQL，右侧展示会话历史，底部展示数据结果，整体布局更偏向分析工作台。
- 会话历史持久化到数据库，支持浏览历史问句、查看完整 Prompt，并将历史 SQL 一键同步回当前编辑区。
- 结果区提供更清晰的数据表格样式与行数标签，便于快速判断执行结果规模。
...
- `http://localhost:3000/nl2data`：NL2DATA 自然语言取数工作台
...
### 7. 推荐演示路径
```text
Mock 接口定义 -> API 接入调试 -> API 转发绑定 -> DB API SQL 封装 -> NL2DATA 自然语言取数 -> 高级编排处理 -> 模型管理配置默认模型 -> AI Chat 自然语言改编排 -> 数据库实例查询验证 -> 输出最终响应
```

### 8. 适合在 README 中补充的截图位置
- 首页 Mock 接口总览页
- API Client 的运行调试界面
- API Forward 的参数映射面板
- 高级编排工作流画布与底部输出预览区
...
- NL2DATA 的 AI Chat + SQL + 会话历史三栏工作区
...
# SQLITE_DB_PATH=/var/lib/api-forge/mock-data.db
```

说明：

- 默认情况下，SQLite 会写入 `DATA_DIR/mock-data.db`
- 如需固定文件路径，可显式设置 `SQLITE_DB_PATH`
- 服务启动时会自动创建数据库所在目录
...
如需完整生产部署步骤，请参考 `docs/deploy-production.md`。
